...
if __name__ == "__main__":
    ...
    if not args.skip_test and args.profiling:
        ...
        plt.plot(
            ...
            memory_alloc_times,
            "s-",
            label=memory_alloc_legend,
            color="orange",
        )
        plt.plot(
            ray_counts, rays_update_times, "^-", label=rays_update_legend, color="green"
        )
        ...
        # 添加总时间曲线
        geom_total_times = [
            p + k for p, k in zip(geom_prepare_times, geom_kernel_times)
        ]
        ...
        # 修改标题，添加测试用的雷达点数
        plt.title(
            f"{geom_count_title} ({test_rays_count} {rays_num_word})", **title_font
        )
        plt.legend(prop={"size": 10})
        plt.grid(True)
        plt.tick_params(labelsize=10)
        ...
        # 修改标题，添加测试用的雷达点数
        plt.title(
            f"{geom_update_title} ({test_rays_count} {rays_num_word})", **title_font
        )
        plt.legend(prop={"size": 9}, loc="upper left")
        plt.grid(True)
        plt.tick_params(labelsize=10)
        ...
        plt.tight_layout()
        if args.save_fig:
            plt.savefig("ti_lidar_performance_analysis.png", dpi=300)
        plt.show()
        ...
        print("-" * 70)
        print(
            f"{'射线数量':^12} | {'准备时间 (ms)':^15} | {'内核时间 (ms)':^15} | {'总时间 (ms)':^15}"
        )
        print("-" * 70)
        for i, count in enumerate(ray_counts):
            total_time = prepare_times[i] + kernel_times[i]
            print(
                f"{count:^12} | {prepare_times[i]:^15.2f} | {kernel_times[i]:^15.2f} | {total_time:^15.2f}"
            )
        print("-" * 70)
    ...
    if use_english_labels:
        ...
        ax.set_title(
            "Lidar 3D Point Cloud Visualization", fontsize=14, fontweight="bold"
        )
    ...
    try:
        ...
        print("=" * 40, "System Info Library", "=" * 40)
        print(system_hardware_details)
    except ImportError:
        print("system-info library not found.")
